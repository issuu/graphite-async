...
module Percentile = struct
  ...
  let expire ~now t =
    t.observations <- List.take_while ~f:(fun (_, ts) -> ts + t.period >= now) t.observations
  ...
  let make_percentile_key period value =
    Printf.sprintf "percentiles.%ds.%s" period value
  ...
  let get_stats ~now t =
    ...
    let get_percentile_value percentile =
      ...
      | n ->
          let ceil_index = (percentile *. Float.of_int n /. 100.0) |> Float.round_up |> Int.of_float in
          Array.get sorted_observations (ceil_index - 1)
    in

    let percentile_values =
      List.map t.percentiles
        ~f:(fun percentile -> (make_percentile_key_float t.period percentile,
                               get_percentile_value percentile))
    in
    ("sum", t.sum) :: ("observations", t.total_observations) ::
    (make_percentile_key t.period "observations", observation_count) ::
    (make_percentile_key t.period "avg", average sorted_observations) :: percentile_values
...
let send_raw t stats =
  let msg =
    stats
    |> List.map ~f:(fun stat -> Printf.sprintf "%s.%s %d %d" t.prefix stat.path stat.value stat.ts)
    |> String.concat ~sep:"\n"
  in
  Amqp.Exchange.publish t.channel t.exchange ~routing_key:"graphite" (Amqp.Message.make msg) >>= fun `Ok ->
  return ()
...
let send t () =
  ...
  |> (fun acc -> Hashtbl.fold t.metrics ~init:acc ~f:(fun ~key ~data acc ->
...
let report t =
  ...
  let percentile_data =
    ...
    Report.{
      ...
      obs_rate = Float.(of_int fractile.Percentile.total_observations / Time.Span.to_sec time_since_init);
      rate = Some Float.(of_int fractile.Percentile.sum / Time.Span.to_sec time_since_init);
...
let incr ?by ~key t =
  Hashtbl.incr ?by t.metrics key

let decr t ~key value =
  Hashtbl.decr ~by:value t.metrics key

let incr_opt ?by ~key t =
  Option.iter t ~f:(fun t -> incr t ~key ?by)

let decr_opt t ~key value =
  Option.iter t ~f:(fun t -> decr t ~key value)

let set t ~key value =
  Hashtbl.set t.metrics ~key ~data:value

let remove t ~key =
  Hashtbl.remove t.metrics key

let add_percentile_observation t ~key value =
  Hashtbl.find_or_add t.percentile_data key ~default:(fun () -> Percentile.init ~period:t.percentile_period ~percentiles:t.percentiles)
...
module Result = struct
  let time ?graphite ~key ~f v =
    ...
    let%bind r = f v in
    let sub_key = match r with Ok _ -> "ok" | Error _ -> "error" in
    add_percentile_observation_opt graphite ~key:(Printf.sprintf "%s.%s" key sub_key) (timer ());
    return r
end
...
module Option = struct
  let time ?graphite ~key ~f v =
    let timer = timer_ms () in
    let%bind r = f v in
    let sub_key = match r with Some _ -> "ok" | None -> "error" in
    add_percentile_observation_opt graphite ~key:(Printf.sprintf "%s.%s" key sub_key) (timer ());
    return r
end
