...
let assert_stats ?(percentiles=[]) ~now ?total_obs ?avg ?sum ?period_obs t =
  ...
  List.iter percentiles ~f:(fun (percentile, value) -> assert_value ~name:(percentile_key percentile) value);
  Alcotest.(check int) "Too many percentiles returned" (List.length percentiles) (List.length stats - 4)

let test_empty_fractile () =
  let t = Percentile.init ~period:60 ~percentiles:[50.; 100.] in
  assert_stats t ~now:0 ~total_obs:0 ~avg:0 ~sum:0 ~period_obs:0 ~percentiles:[(50., (Some 0)); (100., (Some 0))]
...
let test_fractiles () =
  ...
  assert_stats t ~now:60 ~total_obs:5 ~avg:3 ~sum:15 ~period_obs:5 ~percentiles:[(50., Some 3); (100., Some 5)];
  assert_stats t ~now:90 ~total_obs:5 ~avg:4 ~sum:15 ~period_obs:3 ~percentiles:[(50., Some 4); (100., Some 5)];
  assert_stats t ~now:120 ~total_obs:5 ~avg:0 ~sum:15 ~period_obs:0 ~percentiles:[(50., Some 0); (100., Some 0)];
  ...
  assert_stats t ~now:120 ~total_obs:8 ~avg:0 ~sum:27 ~period_obs:0 ~percentiles:[(50., Some 0); (100., Some 0)];
